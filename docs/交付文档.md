...
| 目录 | 文件 | 用途 |
| --- | --- | --- |
| `00_summary` | `data/final_acceptance_summary.json` | 最终验收摘要，当前 `status=completed`、`gate_failures=[]` |
| `01_final3_collection` | `data/acceptance_direct_final3_delivery_manifest.json` | final3 采集 manifest |
| `01_final3_collection` | `data/acceptance_direct_final3_raw_dataset.jsonl` | final3 原始情报，405 行 |
| `01_final3_collection` | `data/acceptance_direct_final3_cleaned_corpus.jsonl` | final3 清洗语料，268 行 |
| `01_final3_collection` | `data/acceptance_direct_final3_high_risk_corpus.jsonl` | final3 高风险语料，151 行 |
| `01_final3_collection` | `data/acceptance_direct_final3_raw_classifications.jsonl` | final3 raw 级分类，405 行 |
| `01_final3_collection` | `data/acceptance_direct_final3_raw_entities.jsonl` | final3 raw 级实体，1007 行 |
| `01_final3_collection` | `data/acceptance_direct_final3_classifications.jsonl` | final3 cleaned 级分类，268 行 |
| `01_final3_collection` | `data/acceptance_direct_final3_entities.jsonl` | final3 cleaned 级实体，642 行 |
| `01_final3_collection` | `data/acceptance_direct_final3_hydrated_pages.jsonl` | hydrated 网页正文，50 行 |
| `02_evidence_pack` | `data/collection_phase_multi_source_acceptance_pack.jsonl` | 多源验收输入包，500 行 |
| `02_evidence_pack` | `data/collection_phase_multi_source_evidence_pack.jsonl` | joined evidence pack，500 行 |
| `02_evidence_pack` | `data/collection_phase_multi_source_evidence_pack_report.json` | 证据包完整性报告 |
| `02_evidence_pack` | `data/collection_phase_multi_source_clue_evidence_index.json` | 高质量线索证据索引 |
| `02_evidence_pack` | `data/external_balanced_source_evidence_pack.jsonl` | 四类来源均衡证据包，80 行 |
| `02_evidence_pack` | `data/authorized_source_rerun_pack.jsonl` | 授权源复跑包，80 行 |
| `02_evidence_pack` | `data/external_source_evidence_snapshots/` | raw payload / snapshot 引用文件 |
| `03_manual_eval` | `data/manual_heldout_report.json` | 人工复核 gold 生成报告 |
| `03_manual_eval` | `data/manual_heldout_eval_current.json` | 人工 held-out 分类 / 实体评测 |
| `03_manual_eval` | `data/eval_manual_heldout_clue_recall_report.json` | 人工线索 gold 召回评测 |
| `03_manual_eval` | `data/manual_review/heldout_review_task.csv` | 200 行人工复核过程表 |
| `03_manual_eval` | `tests/evaluation/manual_heldout_classification.jsonl` | 193 行人工确认 gold |
| `03_manual_eval` | `tests/evaluation/manual_heldout_clues.jsonl` | 24 条人工线索 gold |
| `04_stage_corpora` | `data/collection_phase_raw_dataset.jsonl` | 阶段一全量 raw，4163 行 |
| `04_stage_corpora` | `data/cleaning_phase_cleaned_corpus.jsonl` | 阶段二 cleaned，3464 行 |
| `04_stage_corpora` | `data/cleaning_phase_high_risk_corpus.jsonl` | 阶段二高风险，1095 行 |
| `04_stage_corpora` | `data/classification_extraction_phase_classifications.jsonl` | 阶段三分类，3464 行 |
| `04_stage_corpora` | `data/classification_extraction_phase_entities.jsonl` | 阶段四实体，21774 行实体 |
| `05_model_ocr_benchmark` | `data/eval_llm_ablation.json`、`data/eval_llm_hard_ablation.json`、`data/latest_llm_value_report.json` | LLM 价值门控 |
| `05_model_ocr_benchmark` | `data/ocr_hardset_report.json` | OCR hardset 报告 |
| `05_model_ocr_benchmark` | `data/scale_benchmark_report.json` | 本地规模 benchmark |
...
| multi-source acceptance pack | 当前共享输出包 500 行 |
