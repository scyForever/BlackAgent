...
> 本文对照课题文档的「分阶段目标」「核心挑战」两张表，逐项说明**怎么做的**、**用了什么技术、为什么这么选**，并给出对应的代码模块、配置和已核对指标。
...
| 维度 | 内容 |
| --- | --- |
| 怎么做的 | 按 IM/群组、社媒/论坛、垂直/技术、公众号/文章的来源清单采集；每个 source 标注 `source_class`（）； query rewrite ；分层任务队列 / 调度器做 cron/queue 风格分批采集。 |
| 技术选型 | 公开页 / 搜索页经 `r.jina.ai` + DuckDuckGo 取摘要；X、Telegram 走各自授权客户端。每条 raw 保留 `source_url`、`raw_payload_uri`、`crawl/publish time`，为后续证据链留锚点。 |
| 为什么这么选 | 课题要求合规采集——不能购买数据、绕过登录、进入私群或恶意抓取。因此用「公开/授权来源 + 来源策略硬校验」而非通用爬虫。 |
| 合规闸 | 禁止绕过登录/验证码/私群，拦截 URL 中的 token/cookie/session/basic-auth。 |
| 产出与证据 | 全量 4163 raw / 83 来源；IM/群组 3786、社媒/论坛 356、垂直/技术 21。final3 主包 405 raw。 |

### 2.2 智能清洗 —— 自动去重、过滤噪声、识别高危内容

| 维度 | 内容 |
| --- | --- |
| 怎么做的 | 文本归一化、稳定去重分组、香农熵噪声分、质量分、风险信号画像。 |
...
| 维度 | 内容 |
| --- | --- |
| 怎么做的 | 规则分类；分类前置 `RiskPolarityScorer`（极性判别，区分公告/反诈/研究/否定语境）；**分类仲裁**：保留 `classification.rule / llm / final / resolution` 四层，LLM 不直接覆盖规则结果，下游只消费 `final`，并保留 `strategy / reason / review_required` 供人工复核与审计。 |
| 标签体系 | 一级风险词、promotion marker、二级标签，经（`RuleRegistry`）统一加载，可配置扩展；评估输出 `rule_version` 便于定位规则版本影响。 |
...
| 维度 | 内容 |
| --- | --- |
| 怎么做的 | 正则 + 词典抽取；`EntityNormalizer`统一**邀请码、TG、URL、联系方式**的 `normalized / hash / masked` 字段；去噪去重、给实体相关性打分。 |
| 技术选型 | **正则 + 词典 + 归一化打底，可选本地 BERT NER 适配**（默认不下载模型、不新增依赖，未配置时回退确定性规则）。 |
...
| 安全 | 落库联系方式/账号的 `canonical_hash` 可加盐（`BLACKAGENT_PII_HASH_SALT`），并做掩码。 |
...
- **动态词库**：提供黑话/谐音/主题同义词；黑话候选发现 + 灰度生命周期。
- **语义归一化**：`EntityNormalizer` 把同一实体的变体（邀请码、TG handle、URL、联系方式）归一为 canonical 形式，再 hash/mask；处理「语义隐蔽」中的反讽/防御/研究语境。
- **多模态隐蔽**：把图片文字纳入分析范围，OCR hardset 20 条覆盖 chat/poster/qr/screenshot，子串命中 20/20。

### 3.3 挑战③：效果 / 成本 / 时延三角平衡 → LLM + NLP 协同 + Agent 编排

- **协同策略**：简单 query 走规则 parser；复杂 query / runtime 黑话上下文 / live source 规划才走 LLM parser/plan。`PreflightIntent`在 LLM 前先抽需求。
- **预算与路由**：fast/balanced/high_recall 三种模式 + 统一控制调用次数、token、候选条数、时延预算。
- **LLM 价值门控**：**选择开开 LLM 全量增强**。
...
| 决策 | 选了什么 | 为什么 | 取舍 / 边界 |
| --- | --- | --- | --- |
| 整体形态 | 本地 CLI / 进程内 runtime，不起公网服务 | 课题是分析 Agent 非线上风控；本地可复跑便于验收 | `serve_demo_api.py` 仅 demo，不代表生产 API |
...
| LLM 用法 | 受控增强 + value gate | 实测 record-enrich 无质量收益却增成本 | 不声称「全量 LLM 提升」 |
...
1. 全量来源偏 IM/Telegram 公开目录，不是天然均衡。
2. OCR / 黑话候选目前是受控 hardset / probe 证明，不是生产泛化或大规模人工确认。
3. 系统输出**人工复核候选**，不自动定性、不自动封禁处置、不覆盖私群 / 登录后页面 / 验证码绕过 / 购买数据。
