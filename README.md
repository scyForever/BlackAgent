...
- **本地 clue pipeline**：`IntelligencePipeline` 已接入 Clean/Dedup/Classify/Extract/LLMEnrich/Correlate/CluePromotion/Score 真实 stage；分类前置 `RiskPolarityScorer` 区分公告/反诈/研究/否定语境，抽取侧通过 `EntityNormalizer` 统一邀请码、TG、URL、联系方式的 normalized/hash/masked 字段。
