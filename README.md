...
- **本地 clue pipeline**：`IntelligencePipeline` 已接入 Clean/Dedup/Classify/Extract/Correlate/Score 真实 stage；`OfflineClueBuilder` 优先走该 stage pipeline，必要时回退旧 `PhaseTwoThreeEngine`。
- **LLM 预算与路由**：`routing_profiles.yaml + ModelRouter + BudgetController + ClueRanker` 统一控制 intent/plan/query rewrite/clue refine 的调用、token、候选条数和时延预算。
...
python scripts/evaluate_pipeline.py --gold tests/evaluation/gold_classification.jsonl --output data/eval_report.json
```
